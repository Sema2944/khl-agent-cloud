...
# -----------------------------
# LIVE snapshots (MVP) — на user_id+match_id
# -----------------------------
...
def _line_hash_for_cache(match_id: str, payload: Dict[str, Any]) -> str:
    raw = json.dumps({"match_id": match_id, "payload": payload}, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def _line_snapshot_for_mode(mode: str) -> Dict[str, Any]:
    """
    В демо: PRE содержит и коэффициенты, LIVE — без коэффициентов (только параметры).
    """
...
def _build_ui_prompt(match_id: str, mode: str, action: str, prev_snap: Optional[Dict[str, Any]], cur_snap: Dict[str, Any]) -> str:
    ...
    if mode == "live":
        base += [
            "",
            "Задача:",
            "Сформируй LIVE-обзор: 3–5 пунктов контекста и 1–3 ключевых рынка (Total/Handicap).",
            "Если есть предыдущий снапшот — объясни направление изменения (up/down/flat) и почему, без кэфов.",
            "",
            "Верни СТРОГО JSON с полями:",
            '{"title":"🟢 LIVE-обзор","context":["..."],"markets":[{"name":"Total|Handicap","direction":"up|down|flat|unknown","logic":"..."}],"risks":["..."],"disclaimer":"..."}',
        ]
    else:
        base += [
            "",
            "Задача:",
            "Сформируй PREMATCH-разбор (обзор/рынок): summary + key_factors + line_logic + risks.",
            "",
            "Верни СТРОГО JSON с полями:",
            '{"title":"📊 Обзор рынков","summary":"...","key_factors":["..."],"line_logic":["..."],"risks":["..."],"disclaimer":"..."}',
        ]
...
def _render_ui_json(analysis: Any, mode: str) -> str:
    ...
    if mk:
        lines.append("")
        lines.append("*Ключевые LIVE-рынки*")
        for item in mk[:3]:
    ...
    lines.append("")
    lines.append(f"_{str(disclaimer).strip('_')}_")
    return "\n".join(lines)


async def ai_analyze_legacy(*, user_id: int, match_id: str, market_key: str) -> str:
    """
    Оставляем legacy-команду `аналитика <id> <market>` для совместимости.
    """
    _ = user_id
    m = _find_match(match_id)
    ...
    domain_prompt = "\n".join(
        [
            ...
            f"Матч: {m['title']} ({m['league']})",
            f"match_id: {match_id}",
            f"Рынок: {market['label']} ({market_key})",
            f"Линия(JSON): {json.dumps(data, ensure_ascii=False)}",
            "",
            "Нужно: 3–5 тезисов, 2–4 риска, 2–4 пункта чек-листа.",
        ]
    )

    cache_payload = {"legacy": True, "market_key": market_key, "line": data}
    cache_key = f"v1:legacy:{match_id}:{market_key}:{_line_hash_for_cache(match_id, cache_payload)}"
    analysis, meta = await analyze_with_llm_cached(domain_prompt, cache_key=cache_key, schema="legacy")

    logger.info("LLM meta(legacy): %s", {k: meta.get(k) for k in ("provider", "attempts", "elapsed_ms", "used_fallback", "last_error", "cache")})
    return render_analysis_text(analysis)  # type: ignore[arg-type]
...
async def run_dialog_agent(user_id: int, message: str) -> str:
    ...
    # --- Диагностика
    if norm == "version":
        return "✅ parsing.py version: 2025-12-26 v3 (ui pre/live + llm ping + env)"
    ...
    if norm == "llm ping":
        prompt = "Верни корректный JSON по ui_pre схеме. Заголовок '📊 Обзор рынков'."
        analysis, meta = await analyze_with_llm_cached(
            prompt,
            cache_key=f"diag:ping:{_now_ts()}",
            schema="ui_pre",
            ttl_s=0,  # всегда мимо кэша, чтобы видеть актуальную ошибку
        )
        return (
            "🧪 *LLM ping*\n"
            ...
            f"• sample_title: `{(analysis.get('title') if isinstance(analysis, dict) else '-')}`"
        )

    # --- UI actions (из кнопок)
    ...
    if norm.startswith("ui match"):
        ...
        if mode == "live":
            ...
            if action == "refresh":
                ...
                # если первый заход — запоминаем снапшот
                if k not in _LIVE_SNAPSHOT_BY_USER_MATCH:
                    _LIVE_SNAPSHOT_BY_USER_MATCH[k] = {"ts": _now_ts(), "line": cur_snap}
        ...
        base_payload = {"match_id": match_id, "mode": mode, "action": action, "cur": cur_snap, "prev": prev_snap}
        h = _line_hash_for_cache(match_id, base_payload)
        suffix = f":r{_now_ts()}" if force_refresh else ""
        cache_key = f"v3:ui:{match_id}:{mode}:{action}:{h}{suffix}"
        ...
        logger.info("LLM meta(ui): %s", {k: meta.get(k) for k in ("provider", "attempts", "elapsed_ms", "used_fallback", "last_error", "cache")})
    ...
    if norm.startswith("матчи сегодня"):
        ...
        if not sport:
            return "Напиши: `матчи сегодня hockey` (hockey/football/basketball/tennis/esports)"
    ...
    if norm.startswith("рынок"):
        ...
        return "\n".join(
            [
                ...
                f"Матч: {m['title']} ({m['league']})",
                f"market: `{market_key}`",
                f"line: `{json.dumps(market['data'], ensure_ascii=False)}`",
                "",
                f"Дальше: `аналитика {match_id} {market_key}`",
                "_Дисклеймер: показано для объяснения рынков. Не является рекомендацией._",
            ]
        )
    ...
    if norm.startswith("аналитика"):
        ...
        parts = body.split()
        if len(parts) >= 2:
            return await ai_analyze_legacy(user_id=user_id, match_id=parts[0], market_key=parts[1])
