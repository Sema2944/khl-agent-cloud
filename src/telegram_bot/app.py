...
# -----------------------------
# Эталонные тексты экранов
# -----------------------------
MARKET_OVERVIEW_TEXT = (
    "📊 *Обзор рынков*\n\n"
    "Выбери, какой рынок разобрать 👇\n\n"
    "*1X2 (исход)*\n"
    "• кто контролирует матч\n"
    "• перекос линии / value\n"
    "• особенно полезно в LIVE\n\n"
    "*Тоталы*\n"
    "• зависят от темпа и состава\n"
    "• по ходу матча линия меняется быстро\n\n"
    "*Фора*\n"
    "• когда есть разница в классе/давлении\n"
    "• удобно в связке с LIVE\n\n"
    "*LIVE*\n"
    "• основная зона value\n"
    "• следим за динамикой и моментумом\n\n"
    "_Аналитический материал, не является рекомендацией._"
)
...
def _premium_text(tg_user_id: int) -> str:
    ...
    return (
        ...
        "• больше AI-лимитов\n"
        "• расширенные фичи (дальше добавим)\n"
    )
...
def _sync_user(tg) -> None:
    # ✅ строго по сигнатуре user_store.get_or_create_user()
    get_or_create_user(
        int(tg.id),
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "BACK:MATCH_HUB":
        ...
        if not match_id:
            await _edit_or_send(screen_msg, "🏟 Выбери спорт:", reply_markup=kb_sports())
            return
        await _edit_or_send(screen_msg, "Выбери действие:", reply_markup=kb_match_hub(match_id))
        return
    ...
    if data == "PAY:PREMIUM":
        await _edit_or_send(screen_msg, PREMIUM_SOON_TEXT, reply_markup=kb_premium())
        return
