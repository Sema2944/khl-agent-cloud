...
def _env_int(name: str, default: int) -> int:
    ...
    try:
        ...
        logger.warning("Bad int env %s=%r, using default=%s", name, raw, default)
        return default
...
def _env_float(name: str, default: float) -> float:
    ...
    try:
        ...
        logger.warning("Bad float env %s=%r, using default=%s", name, raw, default)
        return default
...
LLM_429_COOLDOWN_DEFAULT_S = _env_int("LLM_429_COOLDOWN_DEFAULT_S", 600)
LLM_429_COOLDOWN_MAX_S = _env_int("LLM_429_COOLDOWN_MAX_S", 3600)

# soft limits / housekeeping
GW_MAX_CACHE_ITEMS = _env_int("GW_MAX_CACHE_ITEMS", 2000)
GW_CLEANUP_EVERY_S = _env_int("GW_CLEANUP_EVERY_S", 30)

# NOTE: if you already added these in your newer gateway version, keep them.
# They will not break older behavior.
GW_RPS_PRE = _env_float("GW_RPS_PRE", 1.0)       # requests per second (pre)
GW_RPS_LIVE = _env_float("GW_RPS_LIVE", 1.5)     # requests per second (live)
...
def _parse_retry_after(headers: Dict[str, str]) -> Optional[int]:
    """
    retry-after may be seconds string.
    """
    if not headers:
        return None
    ...
    if not ra:
        return None
    try:
        sec = int(float(str(ra).strip().replace(",", ".")))
...
class LLMGateway:
    ...
    def __init__(self, provider: str):
        ...
        logger.info(
            "LLM gateway initialized: provider=%s GW_RPS_PRE=%s GW_RPS_LIVE=%s",
            provider,
            GW_RPS_PRE,
            GW_RPS_LIVE,
        )
