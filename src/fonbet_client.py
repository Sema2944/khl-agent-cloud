...
# ------------------ ЛОКАЛЬНЫЙ ТЕСТ "С ПРИНТОМ" ------------------


async def _demo_print_events() -> None:
    """
    Простейший тест: выводим в консоль список KHL-событий
    с 1–2 маркетами, чтобы убедиться, что линия тянется.
    """
    events = await get_khl_events_for_today()

    if not events:
        print("Нет KHL событий в линии.")
        return

    print(f"Найдено KHL событий: {len(events)}")
    print("-" * 40)

    for e in events[:5]:  # печатаем максимум 5 матчей
        print(f"ID: {e.id} | {e.team1} — {e.team2} | Лига: {e.league}")
        # чуть-чуть маркетов покажем
...
if __name__ == "__main__":
    asyncio.run(_demo_print_events())
