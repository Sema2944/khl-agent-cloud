...
# Для SQLite + aiosqlite строка ДОЛЖНА быть в async-формате:
# sqlite+aiosqlite:///./data.sqlite3
...
async def init_db() -> None:
    """Создаём async-движок и фабрику сессий, мигрируем таблицы."""
    global _engine, _session_factory

    # create_async_engine — единственно верный путь для async
...
def async_session() -> AsyncSession:
    assert _session_factory is not None, "DB is not initialized"
...
async def upsert_events(events: List[OddsEvent]) -> int:
    ...
    async with async_session() as s:
        inserted = 0
        for ev in events:
            res = await s.execute(select(OddsEvent).where(OddsEvent.ext_id == ev.ext_id))
...
async def search_events(query: Optional[str] = None, limit: int = 10) -> List[OddsEvent]:
    async with async_session() as s:
        if query:
            like = f"%{query.lower()}%"
            q = (
