...
# HEALTHCHECK ДЛЯ RENDER: GET + HEAD
...
# === ДАЛЬШЕ ИДЁТ ВСЁ, ЧТО У ТЕБЯ УЖЕ БЫЛО В ЭТОМ ФАЙЛЕ ===
# (парсер ставок, отчёты, build_user_profile, build_value_analysis,
#  build_express_evaluation и в самом конце run_agent)
# Я их не вырезаю/не меняю — просто оставь как есть ниже.
# Ниже приведу кусок с run_agent, чтобы точно был:
...
async def run_agent(user_id: int, message: str, session: Session) -> str:
    ...
    # ... (и далее весь тот большой if/else, который у тебя уже есть:
    # банк, отчёты, КХЛ сегодня, экспресс, добавление ставок и т.д.)

    # В самом конце — заглушка help по умолчанию:
