...
def _get_all_bets(session: Session, user_id: int):
    """
    Все ставки пользователя (для анализа рынков).
    """
...
def build_user_market_insights(session: Session, user_id: int) -> str:
    ...
    """
    bets = _get_all_bets(session, user_id)
    if not bets:
        return (
...
async def run_agent(user_id: int, message: str, session: Session) -> str:
    ...
    # 5) ОБЩАЯ СТАТИСТИКА ПОЛЬЗОВАТЕЛЯ
    if (
        "статист" in text
        or "статку" in text
        or "stats" in text
    ...
    # 11) ЗАГЛУШКИ ПОД БУДУЩИЕ РАЗДЕЛЫ
    if "аналити" in text and "матч" in text:
        return (
            "Раздел аналитики матчей расширяется.\n"
            "Уже сейчас можно:\n"
