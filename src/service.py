...
def build_khl_match_analysis(event) -> str:
    """
    Разбор матча КХЛ по данным события:
    - линия 1X2 с имплайд-вероятностями и маржой
    - основной тотал, если найден
    ...
    for m in markets:
        ...
        # Классический рынок исхода матча
        if name in ("1X2", "1X", "3WAY", "3-WAY"):
            market_1x2 = m

        # Рынок тотала (берём первый подходящий)
        if market_total is None and any(k in name for k in ("TOTAL", "ТОТАЛ")):
            market_total = m
    ...
    if market_1x2:
        ...
        if odds_list:
            ...
            if ratio is not None:
                ...
                lines.append(
                    "• Фаворит и андердог по линии определяются, но коэффициенты выглядят странно."
                )
    ...
    try:
        form1: TeamForm | None = get_team_form(team1)
    except Exception:
        form1 = None
    try:
        form2: TeamForm | None = get_team_form(team2)
    except Exception:
        form2 = None
