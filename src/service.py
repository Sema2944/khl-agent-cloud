...
def build_value_analysis(raw_text: str) -> str:
    ...
    Без доступа к истории ставок, просто:
    - достаём кэф из текста
    - считаем имплайд-вероятность
    - даём чек-лист, как думать о value.
    """
    # Пытаемся вытащить любое число как коэффициент
    _, odds = _parse_stake_and_odds(raw_text)
    ...
    if odds < 1.40:
        ...
        lines.append(
            ...
            "всё решает твоя оценка вероятности."
        )
    else:
        lines.append(
    ...
    lines.append("")
    lines.append(
        "Важно: сам по себе коэффициент не 'хороший' и не 'плохой'. "
        "Value появляется только тогда, когда твоя оценка вероятности устойчиво "
        "лучше, чем у рынка на длинной дистанции."
    )
