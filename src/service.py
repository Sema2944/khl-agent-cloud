...
@app.get("/healthz")# src/service.py
from __future__ import annotations
import asyncio
import os
...
async def _run_bot_polling():
    """Фоновая задача: запустить polling и держать бота активным, не закрывая event loop Uvicorn."""
    global _bot_app
    ...
    # ВАЖНО: не перехватывать сигналы и не закрывать event loop веб-сервера
