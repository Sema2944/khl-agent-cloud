...
async def _collect_odds(
    ...
    # --- Attempt 1: The Odds API (skip for unsupported leagues like KHL) ---
    from .odds_client import should_skip_odds_api
    if should_skip_odds_api(sport_slug, league):
        logger.info("_collect_odds: skipping The Odds API for %s (league='%s' not supported)",
                     ctx.match_id, league)
    else:
        try:
